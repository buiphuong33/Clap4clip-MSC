...
def parse_option():
    ...
    # --- Semantic Drift Calibration flags (MSC/CC) ---
    parser.add_argument("--use-msc", action="store_true", default=False,
                        help="Enable Mean-Shift Compensation on text-side class prototypes")
    parser.add_argument("--msc-momentum", type=float, default=0.5,
                        help="EMA factor for updating class means (0..1). Higher = trust old means more")

    parser.add_argument("--use-cov-calib", action="store_true", default=False,
                        help="Enable Covariance Calibration loss with Mahalanobis constraint")
    parser.add_argument("--lambda-cov", type=float, default=0.1,
                        help="Weight for covariance calibration loss")

    # tùy chọn: nơi lấy thống kê lớp sau train (train_loader hiện tại là ổn)
