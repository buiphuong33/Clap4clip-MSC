    ...
    def forward(self, image, labels=None, test=False, finetuning=False, return_mean=True, for_prior=None):
        ...
        if test:
            with torch.no_grad():
                ...
                need_text_feats = (self.args.compute_ram 
                   or getattr(self.args, "use_msc", False) 
                   or getattr(self.args, "use_cov_calib", False))
                rsamples_g = None 
...
class ClClipVariational(Evaluator):
    ...
    def fit(self, data):
        ...
        self._last_train_loader = train_loader  # để post_training dùng tính stats
    ...
    def post_training(self, finalize=False):
        ...
        self.model.eval()
        if not finalize and (getattr(self.args, "use_msc", False) or getattr(self.args, "use_cov_calib", False)):
    ...
    def init_model(self, class_names, per_epoch_steps, prompt_templates=None):
        ...
        # cho phép CLIP.forward truy cập stats & args để MSC/CC khi cần
        self.model.class_stats = self.class_stats
        self.model.args = self.args  # đảm bảo model thấy flags use_msc/use_cov_calib
    ...
    # (2) Thêm vào thân class ClClipVariational (cùng cấp với các method khác):

    @torch.no_grad()
    def compute_text_side_stats(self, data_loader):
        """
        Chạy 1 pass test để lấy 'text-side class features' sau VGA/adapters (đúng phần dễ trôi).
        Gom theo nhãn để ước lượng mu, cov từng lớp.
        """
        if data_loader is None:
            return {}
        self.model.eval()

        per_cls = {}  # cid -> list[tensor D]
        for x, y, _ in data_loader:
            x = x.cuda(device=self.args.default_gpu)
            y = y.cuda(device=self.args.default_gpu)
            # test=True, return_mean=False để nhận cả (visual, text_feats)
            logits, (visual_feats, text_feats) = self.model(x, y, test=True, return_mean=False)
            # text_feats có shape [B, D] - 1 vector/prototype cho mỗi sample theo nhãn
            for f, c in zip(text_feats, y):
                c = int(c.item())
                per_cls.setdefault(c, []).append(f.detach().cpu())

        stats = {}
        for c, feats in per_cls.items():
            Fmat = torch.stack(feats, 0)  # [N_c, D]
            mu = Fmat.mean(0)
            X = Fmat - mu
            # cov không lệch + ổn định
            denom = max(1, (Fmat.shape[0] - 1))
            cov = (X.t() @ X) / denom
            cov = cov + 1e-5 * torch.eye(cov.shape[0])
            stats[c] = {"mu": mu, "cov": cov}
        return stats

    def apply_msc_update(self, new_stats):
        """
        Cập nhật mean theo EMA (ổn định) + thay cov bằng cov mới đo được.
        """
        m = getattr(self.args, "msc_momentum", 0.5)
        for c, s in new_stats.items():
            if c in self.class_stats and "mu" in self.class_stats[c]:
                old_mu = self.class_stats[c]["mu"]
                mu = m * old_mu + (1.0 - m) * s["mu"]
                self.class_stats[c]["mu"] = mu
                self.class_stats[c]["cov"] = s["cov"]
            else:
